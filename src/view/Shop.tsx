...
const Shop = () => {
  const { searchTypeBar, searchNameAndType, getInput } = useSearchContext();
  ...
  const NameorderCartall = Storeitems.filter(
    (e) => e.name.toLowerCase().slice(0).includes(getInput) //เปลี่ยนค่ากงนี้
  );
  ...
  return (
    <div>
      ...
      <div className=" flex flex-wrap gap-6 items-center mt-[40px] xl:max-w-7xl xl:justify-start xl:m-[auto] justify-center ">
        {getInput.length > 0 && NameorderCartall.length===0 && searchNameAndType === true ?<>
